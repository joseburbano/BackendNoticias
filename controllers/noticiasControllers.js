...
exports.inicioNoticias = async (req, res) => {
  const noticiasDatos = [];
...
//funcion 
  await (async function () {
    for await (let num of recorrido) {
      await num.paginate({}, options, (err, storedP) => {
        if (err) {
          res.status(500).json({ code: 500, message: "Error del servidor." });
        } else {
          if (!storedP) {
            res.status(404).json({
              code: 404,
              message: "No se ha encontrado ninguna noticias.",
            });
          } else {
            noticiasDatos.push(storedP);
          }
        }
      });
    }
  ...
  res.status(200).json({ code: 200, noticias: noticiasDatos });
};
